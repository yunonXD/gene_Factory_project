...
public class BattleStage2_3 : MonoBehaviour
{
    //탕그리스 vs 모디비우스
    ...
    public ParticleSystem Skilleffect_3;
    ...
    void Start()
    {
        ...
        Boss.GetComponent<Renderer>().sharedMaterials[1].DOColor(color, "_Color", 1);

        if (player.GetComponent<TanngrisScript>().Agility > enemy.GetComponent<Enemy_Silme>().Agility)
            OrderAttack = true;
        else
            OrderAttack = false;
        ...
        Skilleffect_3.Stop();
        //EnemyReset();                                                       //적 위치 및 
        enemyDamage = enemy.GetComponent<Enemy_Silme>().Power - player.GetComponent<TanngrisScript>().Defense;            //적  공격대미지
        _enemyDamage = Boss.GetComponent<MobydickScript>().Power - player.GetComponent<TanngrisScript>().Defense;
        playerDamage = player.GetComponent<TanngrisScript>().Power - enemy.GetComponent<Enemy_Silme>().Defense;           //플레이어 공격대미지
        _playerDamage = player.GetComponent<TanngrisScript>().Power - Boss.GetComponent<MobydickScript>().Defense;
        Player_HPMax = player.GetComponent<TanngrisScript>().HP;
        Debug.Log(Player_HPMax);
        Enemy_HPMax = enemy.GetComponent<Enemy_Silme>().HP;
        Boss_HPMax = Boss.GetComponent<MobydickScript>().HP;
    ...
    void MovePlayer_Mob()     //잡몹 스테이지 시작시 플레이어 활성화,이동모션 및 이동
    {
        ...
        player.GetComponent<TanngrisScript>().walk();
        player.transform.DOLocalMoveX(-570, 1);             //#001
    }
    ...
    void Characterstartmove_Boss()  //체크 1번
    {
        ...
        player.GetComponent<TanngrisScript>().walk_3();
    }
    ...
    void EnemyReset()  //적 캐릭터(잡몹, 보스 페이드 아웃)
    {
        ...
        Boss.GetComponent<Renderer>().sharedMaterials[1].DOColor(color, "_Color", 0.2f);
    }
    ...
    void Update()
    {
        if (MobClear == false && _GameOver == false)  //잡몹 전투
        {
            ...
            Player_Hpbar.fillAmount = player.GetComponent<TanngrisScript>().HP / Player_HPMax;
            ...
            else
            {
                ...
                {
                    ForBattle_FMod.instance.SkillReady();  //스킬 충전 완료 사운드
        ...
        else if (MobClear == true && _GameOver == false)        //보스 전투
        {
            ...
            {
                Player_Hpbar.fillAmount = player.GetComponent<TanngrisScript>().HP / Player_HPMax;
                Enemy_HPbar.fillAmount = Boss.GetComponent<MobydickScript>().HP / Boss_HPMax;
                Skiil_Bar.fillAmount = skill;
            }
        ...
        if (player.GetComponent<TanngrisScript>().HP <= 0) //아군 캐릭터 사망Characterstartmove_Boss
        {
    ...
    void ClearMotion() //스테이지 클리어 모션연출      위치값 변경 위치#2
    {
        ...
        player.GetComponent<TanngrisScript>().wait_2();
    ...
    void Clearsave()   //클리어후 세이브 데이터 입력 스테이지 클리어 체크 변경 #3
    {
        ...
        SaveData.GetComponent<SaveDataManager>()._Stage2_3 = true;
    ...
    void PlayerAttack_Mob()
    {
        ...
        //player.transform.DOLocalMoveY(-200, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3   //플레이어 공격이동  
        player.GetComponent<TanngrisScript>().attack();
        ...
        ForBattle_FMod.instance.Normal_Attack_tangri();     //탕그리스 공격 사운드
    ...
    {
        Skilleffect_2.transform.DOLocalMoveX(110, 0.1f);
        Skilleffect_2.transform.DOLocalMoveY(15, 0.1f);
        Skilleffect_1.transform.DOLocalMoveX(45, 0.1f);

        Skuilleffect.transform.DOLocalMoveX(-50, 0.1f);
        Skuilleffect.transform.DOLocalMoveY(-30, 0.1f);
        ...
        //player.transform.DOLocalMoveY(-250, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3   #9
        Skilleffect_2.Play();
        Boss.GetComponent<MobydickScript>().damage(); //피격모션
        Boss.GetComponent<MobydickScript>().HP = Boss.GetComponent<MobydickScript>().HP - _playerDamage; //공격시 대미지 계산 후 적군 캐릭터 HP 감소
        PlayerDamageText.GetComponent<DamageScript>().damage(0, 4);
        ...
        Invoke("_camera", 0.2f);
        Invoke("BloodReset", 1f);
        Boss.transform.DOLocalMoveX(850, 0.8f);                 //#10
        //enemy.transform.DOLocalMoveX(705, 0.8f);                //#11
        Invoke("resetposition", 1.0f); // 1.0초 뒤에 원위치 진행    
        Invoke("CameraReset", 1f);

        if (Boss.GetComponent<MobydickScript>().HP <= 0)
        {
            Enemy_HPbar.fillAmount = Boss.GetComponent<MobydickScript>().HP / Boss_HPMax;
            Invoke("EnemyReset", 0.6f);
            Invoke("skeletonMark", 0.6f);
    ...
    {
        player.transform.DOLocalMoveX(-570, 0.5f).SetEase(Ease.OutQuad);    //#001
        player.transform.DOLocalMoveY(-250, 0.5f).SetEase(Ease.OutQuad);
        enemy.transform.DOLocalMoveX(630, 0.5f).SetEase(Ease.OutQuad);
        Boss.transform.DOLocalMoveX(500, 0.5f).SetEase(Ease.OutQuad);       //#004
    }
    ...
    {
        Debug.Log("페이스");
        Battle_Cut_Face.transform.DOLocalMoveX(-52f, 1f);
        Battle_Cut_Emblem.transform.DOLocalMoveX(90.0f, 1f);
    }
    ...
    void SkiilAttack_Boss()
    {
        ...
        Boss.SetActive(true);
        player.SetActive(true);
        player.GetComponent<TanngrisScript>().skill();
        Skilleffect_1.Play();
        //player.transform.DOLocalMoveX(-170, 0.6f); //스킬공격 모션X
        //player.transform.DOLocalMoveY(90, 0.6f); //스킬공격 모션Y
        Boss.GetComponent<MobydickScript>().HP = Boss.GetComponent<MobydickScript>().HP - (_playerDamage * 2); //공격시 대미지 계산 후 적군 캐릭터 HP 감소
        ...
        Invoke("Boss_Blood_1_Fadein", 2.2f);
        Invoke("Boss_Blood_1_FadeOut", 2.7f);
        Invoke("_camera", 2.2f);
        Invoke("damegeBossSkill", 2.2f);
        Invoke("CameraReset", 2f);
        Invoke("SkilleffectOut", 3.0f);
        Invoke("Skilleffectremove", 3.0f);
        _SkillAttack = false;
        if (Boss.GetComponent<MobydickScript>().HP <= 0)
        {
            Enemy_HPbar.fillAmount = Boss.GetComponent<MobydickScript>().HP / Boss_HPMax;
    ...
    void SkillSecondMove()
    {
        ...
        player.transform.DOLocalMoveY(-250, 0.3f); //스킬공격 모션Y
    }
    ...
    void damegeBossSkill() //보스 스킬연출
    {
        ...
        Boss.GetComponent<MobydickScript>().damage();
        PlayerDamageText.GetComponent<DamageScript>().damage(1, 0); // 스킬계수
    ...
    {
        Boss.transform.DOLocalMoveX(500, 0.8f);  //##004
    ...
    void SkilleffectOut() //스킬이펙트 종료
    {
        ...
        Skilleffect_2.Stop();
        Color color = new Color(0f, 0f, 0f, 0f);
        BossEffect.GetComponent<Renderer>().sharedMaterials[0].DOColor(color, "_Color", 0.2f);
        resetposition();
        player.GetComponent<TanngrisScript>().ResetWait_1();
    }
    void EnemyAttack_Mob()  //#14
    {
        ...
        Invoke("_camera", 0.2f);
        player.GetComponent<TanngrisScript>().damage();
        player.GetComponent<TanngrisScript>().HP = player.GetComponent<TanngrisScript>().HP - enemyDamage; //피격시 대미지 계산후 아군 캐릭터 HP 감소
    ...
    void EnemyAttack_Boss()
    {
        ...
        Boss.GetComponent<MobydickScript>().attack();
        Skilleffect_3.Play();
        //Boss.transform.DOLocalMoveX(-100, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3
        Invoke("_camera", 0.2f);
        player.GetComponent<TanngrisScript>().damage();
        player.GetComponent<TanngrisScript>().HP = player.GetComponent<TanngrisScript>().HP - _enemyDamage; //피격시 대미지 계산후 아군 캐릭터 HP 감소
        player.transform.DOLocalMoveX(-735, 0.8f);
        ...
        ForBattle_FMod.instance.Normal_Attack_modibic();  //보스 모디비우스 레벨 3 공격 사운드
