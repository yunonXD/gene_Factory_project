...
public class BattleStage1_3 : MonoBehaviour
{
    ...
    public ParticleSystem Skilleffect_Boss;
    ...
    void Start()
    {
        ...
        if (player.GetComponent<NimpueScript>().Agility > enemy.GetComponent<Enemy_Silme>().Agility)
            OrderAttack = true;
        else
            OrderAttack = false;
        ...
        Skilleffect_Boss.Stop();
        Skuilleffect.Stop();
        Skilleffect.Stop();                                                  //스킬이펙트 정지
        //EnemyReset();                                                       //적 위치 및 
        enemyDamage = enemy.GetComponent<Enemy_Silme>().Power - player.GetComponent<NimpueScript>().Defense;            //적  공격대미지
        _enemyDamage = Boss.GetComponent<TanngrisScript>().Power - player.GetComponent<NimpueScript>().Defense;
        playerDamage = player.GetComponent<NimpueScript>().Power - enemy.GetComponent<Enemy_Silme>().Defense;           //플레이어 공격대미지
        _playerDamage = player.GetComponent<NimpueScript>().Power - Boss.GetComponent<TanngrisScript>().Defense;
        Player_HPMax = player.GetComponent<NimpueScript>().HP;
        Enemy_HPMax = enemy.GetComponent<Enemy_Silme>().HP;
        Boss_HPMax = Boss.GetComponent<TanngrisScript>().HP;
    ...
    void MovePlayer_Mob()     //잡몹 스테이지 시작시 플레이어 활성화,이동모션 및 이동
    {
        ...
        player.GetComponent<NimpueScript>().walk();
        player.transform.DOLocalMoveX(-670, 1);             //#001
    }
    ...
    void Characterstartmove_Boss()  //체크 1번
    {
        ...
        player.GetComponent<NimpueScript>().walk_3();
    }
    ...
    void Update()
    {
        ...
        {
            Player_Hpbar.fillAmount = player.GetComponent<NimpueScript>().HP / Player_HPMax;
        ...
        else if (MobClear == true && _GameOver == false)        //보스 전투
        {
            ...
            {
                Player_Hpbar.fillAmount = player.GetComponent<NimpueScript>().HP / Player_HPMax;
                Enemy_HPbar.fillAmount = Boss.GetComponent<TanngrisScript>().HP / Boss_HPMax;
                Skiil_Bar.fillAmount = skill;
            }
    ...
    void ClearMotion() //스테이지 클리어 모션연출      위치값 변경 위치#2
    {
        ...
        player.transform.DOLocalMoveX(-740, 0.8f);
        MainCamera.transform.DOLocalMoveX(-64, 0.8f); //#003
        MainCamera.transform.DOLocalMoveZ(15, 0.8f);
        player.GetComponent<NimpueScript>().wait_2();
    ...
    void PlayerAttack_Mob()
    {
        ...
        player.transform.DOLocalMoveX(80, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3    //플레이어 공격이동  ##002
        player.transform.DOLocalMoveY(-250, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3   //플레이어 공격이동  
        player.GetComponent<NimpueScript>().attack();
    ...
    void PlayerAttack_Boss()
    {
        ...
        PlayerDamageText.GetComponent<DamageScript>().Reset();
        player.GetComponent<NimpueScript>().attack();
        player.transform.DOLocalMoveX(80, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3    #002
        ...
        if (Boss.GetComponent<TanngrisScript>().HP <= 0)
        {
            Enemy_HPbar.fillAmount = Boss.GetComponent<TanngrisScript>().HP / Boss_HPMax;
            Invoke("EnemyReset", 0.6f);
            Invoke("skeletonMark", 0.6f);
    ...
    void resetposition() //#12 전체 위치값 초기화용
    {
        ...
        player.transform.DOLocalMoveX(-670, 0.5f).SetEase(Ease.OutQuad);    //#001
        player.transform.DOLocalMoveY(-250, 0.5f).SetEase(Ease.OutQuad);
        enemy.transform.DOLocalMoveX(630, 0.5f).SetEase(Ease.OutQuad);
        Boss.transform.DOLocalMoveX(660, 0.5f).SetEase(Ease.OutQuad);       //#004
    }
    ...
    {
        Debug.Log("페이스");
        Battle_Cut_Face.transform.DOLocalMoveX(-28.8f, 1f);
    ...
    void SkiilAttack_Boss()
    {
        ...
        Boss.SetActive(true);
        player.SetActive(true);
        player.GetComponent<NimpueScript>().skill();
        player.transform.DOLocalMoveX(-625, 0.3f);
        Skilleffect.Play();
    ...
    {
        Boss.transform.DOLocalMoveX(660, 0.8f);  //##004
    ...
    {
        Skilleffect.Stop();
        player.GetComponent<NimpueScript>().ResetWait_1();
    }
    void EnemyAttack_Mob()  //#14
    {
        ...
        Invoke("_camera", 0.2f);
        player.GetComponent<NimpueScript>().damage();
        player.GetComponent<NimpueScript>().HP = player.GetComponent<NimpueScript>().HP - enemyDamage; //피격시 대미지 계산후 아군 캐릭터 HP 감소
    ...
    void EnemyAttack_Boss()
    {
        ...
        Skilleffect.Stop();
        Boss.GetComponent<TanngrisScript>().attack();

        //Boss.transform.DOLocalMoveX(480, 0.3f);
        Skilleffect_Boss.Play();
        //Boss.transform.DOLocalMoveX(-340, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3
        Invoke("_camera", 0.5f);
        player.GetComponent<NimpueScript>().damage();
        player.GetComponent<NimpueScript>().HP = player.GetComponent<NimpueScript>().HP - _enemyDamage; //피격시 대미지 계산후 아군 캐릭터 HP 감소
        player.transform.DOLocalMoveX(-735, 0.8f);
