...
public class BattleStage1_1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (player.GetComponent<MushScript>().Agility > enemy.GetComponent<Enemy_Silme>().Agility)
            OrderAttack = true;
        else
            OrderAttack = false;
        ...
        //EnemyReset();                                                       //적 위치 및 
        enemyDamage = enemy.GetComponent<Enemy_Silme>().Power - player.GetComponent<MushScript>().Defense;            //적  공격대미지
        _enemyDamage = Boss.GetComponent<RabbitScript>().Power - player.GetComponent<MushScript>().Defense;
        playerDamage = player.GetComponent<MushScript>().Power - enemy.GetComponent<Enemy_Silme>().Defense;           //플레이어 공격대미지
        _playerDamage = player.GetComponent<MushScript>().Power - Boss.GetComponent<RabbitScript>().Defense;
        Player_HPMax = player.GetComponent<MushScript>().HP;
        Enemy_HPMax = enemy.GetComponent<Enemy_Silme>().HP;
        Boss_HPMax = Boss.GetComponent<RabbitScript>().HP;
    ...
    void MovePlayer_Mob()     //잡몹 스테이지 시작시 플레이어 활성화,이동모션 및 이동
    {
        ...
        player.GetComponent<MushScript>().walk();
    ...
    void Characterstartmove_Boss()  //체크 1번
    {
        ...
        player.GetComponent<MushScript>().walk_3();
    }
    ...
    void Update()
    {
        ...
        else if (MobClear == true && _GameOver == false)        //보스 전투
        {
            ...
            {
                Player_Hpbar.fillAmount = player.GetComponent<MushScript>().HP / Player_HPMax;
                Enemy_HPbar.fillAmount = Boss.GetComponent<RabbitScript>().HP / Boss_HPMax;
                Skiil_Bar.fillAmount = skill;
            }
    ...
    void PlayerAttack_Boss()
    {
        ...
        Boss.GetComponent<RabbitScript>().damage(); //피격모션
        Boss.GetComponent<RabbitScript>().HP = Boss.GetComponent<RabbitScript>().HP - _playerDamage; //공격시 대미지 계산 후 적군 캐릭터 HP 감소
        PlayerDamageText.GetComponent<DamageScript>().damage(0, 4);
        ...
        if (Boss.GetComponent<RabbitScript>().HP <= 0)
        {
            Enemy_HPbar.fillAmount = Boss.GetComponent<RabbitScript>().HP / Boss_HPMax;
            Invoke("EnemyReset", 0.6f);
            Invoke("skeletonMark", 0.6f);
    ...
    void _Battle_Cut_Face()
    {
        ...
        Battle_Cut_Emblem.transform.DOLocalMoveX(91f, 1f);
    }
    ...
    void SkiilAttack_Boss()
    {
        ...
        Skilleffect.Play();
        Boss.GetComponent<RabbitScript>().HP = Boss.GetComponent<RabbitScript>().HP - (_playerDamage * 2); //공격시 대미지 계산 후 적군 캐릭터 HP 감소
        PlayerDamageText.GetComponent<DamageScript>().damage(1, 0); // 스킬계수

        ForBattle_FMod.instance.SA_Mush();  //머쉬 스킬 공격 사운드
    ...
    void damegeBossSkill() //보스 스킬연출
    {
        ...
        Boss.GetComponent<RabbitScript>().damage();
        PlayerDamageText.GetComponent<DamageScript>().damage(1, 0); // 스킬계수
    ...
    {
        Skilleffect.Stop();
        player.GetComponent<MushScript>().ResetWait_1();
    }
    ...
    void EnemyAttack_Boss()
    {
        ...
        Skilleffect.Stop();
        Boss.GetComponent<RabbitScript>().attack();
        Boss.transform.DOLocalMoveX(-340, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3
        Invoke("_camera", 0.2f);
        player.GetComponent<MushScript>().damage();
        player.GetComponent<MushScript>().HP = player.GetComponent<MushScript>().HP - _enemyDamage; //피격시 대미지 계산후 아군 캐릭터 HP 감소
        player.transform.DOLocalMoveX(-735, 0.8f);
        ...
        ForBattle_FMod.instance.normalHitEffect();  //보스 콘레빗 레벨1 공격 사운드
