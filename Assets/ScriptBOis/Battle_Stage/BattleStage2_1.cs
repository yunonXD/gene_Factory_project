...
public class BattleStage2_1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (player.GetComponent<MantiScript>().Agility > enemy.GetComponent<Enemy_Silme>().Agility)
            OrderAttack = true;
        else
            OrderAttack = false;
        ...
        Skilleffect_2.Stop();
        //EnemyReset();                                                       //적 위치 및 
        enemyDamage = enemy.GetComponent<Enemy_Silme>().Power - player.GetComponent<MantiScript>().Defense;            //적  공격대미지
        _enemyDamage = Boss.GetComponent<FrancScript>().Power - player.GetComponent<MantiScript>().Defense;
        playerDamage = player.GetComponent<MantiScript>().Power - enemy.GetComponent<Enemy_Silme>().Defense;           //플레이어 공격대미지
        _playerDamage = player.GetComponent<MantiScript>().Power - Boss.GetComponent<FrancScript>().Defense;
        Player_HPMax = player.GetComponent<MantiScript>().HP;
        Debug.Log(Player_HPMax);
        Enemy_HPMax = enemy.GetComponent<Enemy_Silme>().HP;
        Boss_HPMax = Boss.GetComponent<FrancScript>().HP;
    ...
    void MovePlayer_Mob()     //잡몹 스테이지 시작시 플레이어 활성화,이동모션 및 이동
    {
        ...
        player.GetComponent<MantiScript>().walk();
        player.transform.DOLocalMoveX(-830, 1);             //#001
    }
    ...
    void Characterstartmove_Boss()  //체크 1번
    {
        ...
        player.GetComponent<MantiScript>().walk_3();
    }
    ...
    void Update()
    {
        if (MobClear == false && _GameOver == false)  //잡몹 전투
        {
            ...
            Player_Hpbar.fillAmount = player.GetComponent<MantiScript>().HP / Player_HPMax;
        ...
        else if (MobClear == true && _GameOver == false)        //보스 전투
        {
            ...
            {
                Player_Hpbar.fillAmount = player.GetComponent<MantiScript>().HP / Player_HPMax;
                Enemy_HPbar.fillAmount = Boss.GetComponent<FrancScript>().HP / Boss_HPMax;
                Skiil_Bar.fillAmount = skill;
            }
    ...
    void ClearMotion() //스테이지 클리어 모션연출      위치값 변경 위치#2
    {
        ...
        player.transform.DOLocalMoveX(-1000, 0.8f);
    ...
    void PlayerAttack_Mob()
    {
        ...
        player.transform.DOLocalMoveX(-100, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3    //플레이어 공격이동  ##002
        player.transform.DOLocalMoveY(260, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3   //플레이어 공격이동  
        player.GetComponent<MantiScript>().attack();
        ...
        enemy.transform.DOLocalMoveX(725, 0.8f);    //적 피격 위치값 이동 #7
    ...
    void PlayerAttack_Boss()
    {
        ...
        PlayerDamageText.GetComponent<DamageScript>().Reset();
        player.GetComponent<MantiScript>().attack();
        player.transform.DOLocalMoveX(-100, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3    #002
        player.transform.DOLocalMoveY(260, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3   #9
        ...
        if (Boss.GetComponent<FrancScript>().HP <= 0)
        {
            Enemy_HPbar.fillAmount = Boss.GetComponent<FrancScript>().HP / Boss_HPMax;
            Invoke("EnemyReset", 0.6f);
            Invoke("skeletonMark", 0.6f);
    ...
    {
        player.transform.DOLocalMoveX(-830, 0.5f).SetEase(Ease.OutQuad);    //#001
        player.transform.DOLocalMoveY(260, 0.5f).SetEase(Ease.OutQuad);
        enemy.transform.DOLocalMoveX(630, 0.5f).SetEase(Ease.OutQuad);
        Boss.transform.DOLocalMoveX(630, 0.5f).SetEase(Ease.OutQuad);       //#004
    }
    ...
    {
        Debug.Log("페이스");
        Battle_Cut_Face.transform.DOLocalMoveX(-40f, 1f);
    ...
    void SkiilAttack_Boss()
    {
        ...
        Boss.SetActive(true);
        player.SetActive(true);
        player.GetComponent<MantiScript>().skill();
        Skilleffect_1.Play();
    ...
    void SkillSecondMove()
    {
        ...
        player.transform.DOLocalMoveY(260, 0.3f); //스킬공격 모션Y
    }
    ...
    {
        Boss.transform.DOLocalMoveX(630, 0.8f);  //##004
    ...
    void SkilleffectOut() //스킬이펙트 종료
    {
        ...
        Skilleffect_2.Stop();
        resetposition();
        player.GetComponent<MantiScript>().ResetWait_1();
    }
    void EnemyAttack_Mob()  //#14
    {
        ...
        Invoke("_camera", 0.2f);
        player.GetComponent<MantiScript>().damage();
        player.GetComponent<MantiScript>().HP = player.GetComponent<MantiScript>().HP - enemyDamage; //피격시 대미지 계산후 아군 캐릭터 HP 감소
        player.transform.DOLocalMoveX(-1000, 0.8f);
    ...
    void EnemyAttack_Boss()
    {
        ...
        Boss.GetComponent<FrancScript>().attack();
        Boss.transform.DOLocalMoveX(-100, 0.3f).SetEase(Ease.OutBack); //이동 속도 관련 //공격이동 0.3
        Invoke("_camera", 0.2f);
        player.GetComponent<MantiScript>().damage();
        player.GetComponent<MantiScript>().HP = player.GetComponent<MantiScript>().HP - _enemyDamage; //피격시 대미지 계산후 아군 캐릭터 HP 감소
        player.transform.DOLocalMoveX(-1000, 0.8f);
    ...
    {
        Skuilleffect.transform.DOLocalMoveX(-30, 0.1f);
        Debug.Log("해골이펙트 플레이");
        Skuilleffect.Play();
    }
